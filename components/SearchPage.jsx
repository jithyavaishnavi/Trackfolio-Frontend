...
export default function SearchPage() {
  ...
  // Extract query and date to display in the heading
  const query = searchParams.get("query");
  const date = searchParams.get("date");

  const displayHeading = () => {
    if (query) {
      return `Search results for "${query}"`; // Show company name if present
    } else if (date) {
      return `Search results for ${new Date(date).toLocaleDateString()}`; // Show date if present
    } else {
      return "Completed Drives"; // Default heading when no search query or date
    }
  };
...
>
  <section className="mt-10 mb-12 max-w-6xl mx-auto px-4">
    <h2 className="text-2xl font-bold mb-12 text-[#F5F5DC] text-center">
      {displayHeading()}
    </h2>
    ...
    {loading ? (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {companies.map((drive, idx) => (
          <motion.div
            key={drive.id || idx}
            className="relative bg-black/60 backdrop-blur-lg rounded-2xl p-6 flex flex-col gap-4 shadow-[0_8px_25px_rgba(167,209,108,0.15)] hover:scale-[1.03] transition-transform duration-200 cursor-pointer"
